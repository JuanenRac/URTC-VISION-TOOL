...
- Include `<stddef.h>` explicitly in `src/vision_sensor_link.c` so its
  disconnected-frame check has a definition of `NULL` when compiled as a
  separate translation unit. Fixes the GCC host-test build failure without
  changing receive-path behavior or incrementing either component version.
...
- **New `src/vision_sensor_link.h`/`.c`** (`vision_sensor_link_process_frame()`)
  - found while auditing the code: the real
  dispatch chaining sensor-frame/reading/rate-limiter/diagnostics
  together existed only as a `static` function inside
  ...
  behavior) - same gap and same fix as sibling repo URTC-SMART-RACK's
  own `rack_link.c` this session - so the real MLX9064x receive path is
  ready to wire into a real I2C/SPI ISR the day the PCB exists.
  `tests/test_vision_sensor_scenarios.c` now calls INTO this module
  instead of defining its own private copy of the logic under test.
  `build_firmware.sh`/`.bat`'s host-test step compiles the new file
  alongside the existing ones.
  Verified for real: this session's shell again had no host `gcc`/`cc`
  on `PATH` - compiled and RAN the real host-side test suite (all 6 test
  files, all 5 `src/` modules including the new one) with the
...
tracking document (for the real MLX9064x I2C register map, still
pending real hardware) - that file isn't tracked in this repo. Removed
the dangling pointer; the real reasoning each comment already gave
(v0 convention, not a copy of the real register map, pending real
hardware/datasheet) stays intact and self-sufficient without it.

Also carries a README (6 languages) correction - a stale hardcoded
`FIRMWARE_VERSION_MAJOR/MINOR/PATCH = 0.0.0` in the Directory Structure
comment, from earlier work, bundled here rather than left stranded.

Verified for real: compiled and ran the exact host-test command with
MSVC (`cl.exe`, VS2019 Build Tools) as the substitute host compiler -
this session's shell had no `gcc`/`cc` on `PATH`, same substitute
already used for this repo's own 0.0.4 entry above - `All tests
passed.`, 0 failures.
