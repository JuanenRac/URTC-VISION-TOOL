...
Aucun PCB/schéma n'existe encore pour cette carte (voir `hardware/`) - les fonctionnalités ci-dessous décrivent la conception cible ; la chaîne d'outils firmware et le pipeline de vision côté hôte sont ce qui est réel aujourd'hui.

### Fonctionnalités Clés :
* 🔬 **Perception Double-Modale** — capture synchronisée d'images Thermique et RGB. *(prévu — nécessite le PCB et les capteurs réels)*
* 🌡️ **Thermique Haute Précision** — prise en charge intégrée des capteurs MLX90640/41/42. *(prévu)*
* 🎯 **Alignement Eye-in-Hand** — PnP et AOI (Inspection Optique Automatisée) sub-millimétrique. *(prévu)*
* 📡 **API CAN Unifiée** — intégré de façon transparente au catalogue de 25 outils d'URTC. *(prévu)*
...
* ✅ **Pipeline de traitement `vision_companion`** — un vrai paquet Python fonctionnel : génération synthétique de trames thermiques+RGB, rendu thermique en fausses couleurs, rapport de statistiques, fonctionne de bout en bout sans matériel connecté. *(implémenté — voir COMPAGNON DE VISION ci-dessous)*
...
│       ├── main.py                 # CLI réelle et fonctionnelle (version / selftest)
